...
class NPYDataset():
    ...
    def __getitem__(self, idx):
        ...
        if self.x_semantics is not None:
            ...
            x_semantics = torch.zeros(x_semantics.shape[0], self.no_obj_classes, dtype=torch.float32).\
                scatter_(-1, x_semantics.unsqueeze(-1).type(torch.long), 1.)
            item['x_semantics'] = x_semantics
        return item
    
    
class NPYDataset_with_control():
    def __init__(self, no_obj_classes=42, **kwargs):
        self.no_obj_classes = no_obj_classes
        self.x, _, _, self.vertices_can, self.x_semantics, _, _, self.objects = data_utils.load_data_with_control(**kwargs)

    def __len__(self):
        return len(self.x)

    def __getitem__(self, idx):
        item = {'x': self.x[idx].reshape(-1, 1), 
                'vertices_can': self.vertices_can[idx].reshape(-1, 3), 
