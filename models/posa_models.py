...
from utils import posa_utils


def load_model(model_name='POSA', **kwargs):
    if model_name == 'POSA':
        model = POSA
    elif model_name == 'POSA_control':
        model = POSA_with_control
    else:
...
class POSA(nn.Module):
    ...
    def forward(self, x, vertices=None):
        ...
        # print('x: ', x.shape)               # [64, 655, 43]
        # print('vertics: ', vertices.shape)  # [64, 655, 3]
        # print('z: ', z.shape)               # [64, 256]
        # print('out: ', out.shape)           # [64, 655, 43]
        
        return out, mu, logvar


class POSA_with_control(nn.Module):
    def __init__(self, **kwargs):
        super(POSA_with_control, self).__init__()
        self.encoder = Encoder(**kwargs)
        self.decoder = Decoder(**kwargs)
        self.fc = nn.Linear(kwargs.get('z_dim') + kwargs.get('no_obj_classes'), kwargs.get('z_dim'))
    ...
    def forward(self, x, vertices=None, objects=None):
        mu, logvar = self.encoder(x, vertices)
        z = self.reparameterize(mu, logvar)
        
        z_objects = torch.cat((z, objects), dim=1)
        new_z = self.fc(z_objects)
        
        out = self.decoder(new_z, vertices)
