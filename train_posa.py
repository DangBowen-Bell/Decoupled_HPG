...
from misc.dataset import NPYDataset, NPYDataset_with_control
...
def train():
    ...
    for batch_idx, data in enumerate(train_data_loader):
        ...
        if args.use_control:
            objects = data['objects'].to(device)
            pr_batch, mu, logvar = model(gt_batch, vertices_can, objects)
        else:        
...
if __name__ == '__main__':
    ...
    with threadpool_limits(limits=1, user_api='blas'):
        ...
        output_dir = osp.join(constants.hpg_result_root, 'posa_training', args.train_cfg_name)
        ...
        #* load model
        if args.use_control:
            model = posa_models.load_model('POSA_control', **args_dict).to(device)
        else:        
            model = posa_models.load_model(**args_dict).to(device)
        
        #* model summary
        # summary(model, [(1, 655, 1), (1, 655, 3), (1, 42)], col_names=("input_size", "output_size", "num_params"),  depth=4)
        # summary(model, [(1, 655, 43), (1, 655, 3)], col_names=("input_size", "output_size", "num_params"),  depth=4)
        # exit(0)

        #* load optimizer
        print('Num of trainable param = {}'.format(posa_models.count_parameters(model)))
        ...
        if args.use_control:
            train_dataset = NPYDataset_with_control(**args_dict)
        else:
        ...
        if args.test:
            ...
            if args.use_control:
                test_dataset = NPYDataset_with_control(**args_dict)
            else:
