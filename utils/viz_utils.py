...
from utils import misc_utils, data_utils
...
def render_body(scene, renderer, body_trimesh=None, vertices=None, faces_arr=None, vertex_colors=None, material=None):
    ...
    #* remove material to display color
    material = None
    if isinstance(body_trimesh, list):
        ...
        body_trimesh = trimesh.Trimesh(vertices, faces_arr, vertex_colors=vertex_colors*255, process=False)
...
def render_multi_view(vertices, faces_arr, vertex_colors, 
                      camera_pose, renderer, camera, light_directional, light_point, material, 
                      num_views=4):
    images = []
...
def render_sample(in_batch, vertices, faces_arr, use_semantics=False, make_collage=True, **kwargs):
    ...
    for i in range(in_batch.shape[0]):
        ...
        # white
        # vertex_colors = np.ones((vertices.shape[0], 3)) * np.array([0.95, 0.95, 0.95])
        
        # 0~1
        # x_clip = np.clip(x, 0, 1)      
        # vertex_colors = np.ones((vertices.shape[0], 3)) * np.array([0.4, 0.0, 0.0])
        # vertex_colors[:, 1] = x_clip / 2
        # vertex_colors[:, 2] = (1.0 - x_clip) / 2
        
        x_clip = np.clip(x, 0, 1)      
        vertex_colors = np.ones((vertices.shape[0], 3)) * np.array([0.3, 0, 0])
        vertex_colors[:, 1] = x_clip / 3
        vertex_colors[:, 2] = (1.0 - x_clip) / 3
        
        #! ******************************************************************************************    
        #! debug
        
        # face_colors = np.ones((faces_arr.shape[0], 3)) * np.array(default_color)
        # mask = []
        # for i in range(faces_arr.shape[0]):
        #     if x_contact[faces_arr[i][0]] or x_contact[faces_arr[i][1]] or x_contact[faces_arr[i][2]]:
        #         mask.append(i)
        # face_colors[mask, :3] = [0.0, 0.0, 1.0]

        # body_trimesh = trimesh.Trimesh(vertices, faces_arr, vertex_colors=vertex_colors, face_colors=face_colors*255, process=False)
        # body_mesh = pyrender.Mesh.from_trimesh(body_trimesh)
        
        # scene = create_pyrender_scene(camera, np.matmul(view_rotations[0], camera_pose), light_directional, light_point)
        # scene.add(body_mesh, 'mesh')    
        ...
        #! ******************************************************************************************
        images_contact = render_multi_view(vertices, faces_arr, vertex_colors, 
                                           camera_pose, renderer, camera, light_directional, light_point, material)
        ...
        if use_semantics:
            ...
            vertex_colors[x_semantics!=0, :3] = np.take(semantics_color_coding,
                                                        ...
                                                        axis=0) / 255.0
            images_semantics = render_multi_view(vertices, faces_arr, vertex_colors, 
                                                 camera_pose, renderer, camera, light_directional, light_point, material)
            images['semantics'] = images_semantics

        # concatnate into one image
        if make_collage:
            l = []
...
def get_semantics_color_coding():
    # matter_port_label_filename = './other/mpcat40.tsv'
    matter_port_label_filename = osp.join(constants.proxe_root, 'mpcat40.tsv')
    df = pd.read_csv(matter_port_label_filename, sep='\t')
    color_coding_hex = list(df['hex'])
